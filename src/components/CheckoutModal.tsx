...
import { X, CreditCard, Smartphone, MapPin, User, Phone } from 'lucide-react';
    ...
    return (
      <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl w-full max-w-md p-8 text-center">
          ...
          <h2 className="text-2xl font-bold text-african-brown mb-4">Order Confirmed!</h2>
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            {[1, 2, 3].map((s) => (
              <div key={s} className="flex items-center">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-semibold ${
                  s <= step ? 'bg-african-terracotta text-white' : 'bg-gray-200 text-gray-600'
                }`}>
                  {s}
                </div>
                {s < 3 && <div className={`w-12 h-1 mx-2 ${s < step ? 'bg-african-terracotta' : 'bg-gray-200'}`} />}
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="p-6">
          {step === 1 && (
            <div>
              <h3 className="text-lg font-semibold text-african-brown mb-4">Order Summary</h3>
              <div className="space-y-3 mb-6">
                {items.map((item) => (
                  <div key={item.product.id} className="flex items-center space-x-3 p-3 bg-african-cream rounded-lg">
                    <img
              ...
              <div className="border-t pt-4">
                <div className="flex justify-between items-center text-xl font-bold">
                  <span>Total:</span>
                  <span className="text-african-terracotta">KSh {total.toLocaleString()}</span>
                </div>
              </div>
            </div>
          )}
          ...
          {step === 2 && (
            <div>
              <h3 className="text-lg font-semibold text-african-brown mb-4">Delivery Information</h3>
              <div className="space-y-4">
                ...
                {orderData.deliveryOption === 'pickup' && (
                  <div className="bg-african-cream p-4 rounded-lg">
                    <h4 className="font-semibold text-african-brown mb-2">Pickup Location:</h4>
                    <p className="text-sm text-gray-700">
                      Mazuri Stores<br />
                      Ukunda-Ramisi Rd, Ukunda, Kenya<br />
          ...
          {step === 3 && (
            <div>
              <h3 className="text-lg font-semibold text-african-brown mb-4">Payment Method</h3>
              <div className="grid grid-cols-2 gap-4 mb-6">
                <label className={`p-4 border-2 rounded-lg cursor-pointer transition-all duration-300 ${
              ...
              <div className="bg-african-cream p-4 rounded-lg mb-6">
                <h4 className="font-semibold text-african-brown mb-2">Order Summary</h4>
